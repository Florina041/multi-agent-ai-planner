...
class ResponseAgent(BaseAgent):
    ...
    def run(
        ...
        selected = next((opt for opt in options if opt.plan_id == decision.selected_plan_id), options[0])
        ...
        warnings = analysis.risks[:] if analysis.risks else ["No critical risks detected, but weekly consistency is essential."]
